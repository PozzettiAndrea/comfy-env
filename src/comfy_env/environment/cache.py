...
def read_comfyui_version(comfyui_dir):
    """The host ComfyUI's own version string, or None.

    ``comfyui_version.py`` is generated by ComfyUI's build from its
    pyproject, so it exists in release trees and git checkouts alike. It is
    preferred over a git sha because a user's ComfyUI is frequently not a
    checkout at all, and over parsing pyproject because the generated file is
    what the running code reports about itself. Never raises.
    """
    if not comfyui_dir:
        return None
    path = Path(comfyui_dir) / "comfyui_version.py"
    try:
...
def write_env_stamp(env_manifest_dir, torch_pin=None, provenance="unknown",
    ...
    stamp = {
        ...
        # What ComfyUI this env was built against, and the host-derived pins
        # it was built to match. Nothing recorded these before, so no env
        # could say what it expected and nothing could report drift. Both are
        # informational: compatibility is decided at runtime by the protocol
        # level and the contract check, not by a version string in a file.
        "comfyui_version": comfyui_version,
        "host_derived": host_derived or {},
    }
    try:
...
def describe_env_stamp_drift(env_manifest_dir, comfyui_dir, host_derived=None):
    """Human-readable drift between what an env was built against and the
    host it is about to serve. Returns None when there is nothing to say.

    Deliberately not part of ``validate_env_stamp``: none of this is fatal.
    An env built against a different ComfyUI or a different comfy-aimdo pin
    usually still works, and whether it does is decided at runtime by the
    protocol level. But it was previously invisible, which is how two envs
    on this machine ran a different memory manager than their host without
    anyone knowing.
    """
    import json as _json

    path = Path(env_manifest_dir) / _STAMP_FILE
    try:
        stamp = _json.loads(path.read_text(encoding="utf-8"))
    ...
    notes = []
    built_cv = stamp.get("comfyui_version")
    live_cv = read_comfyui_version(comfyui_dir)
    if built_cv and live_cv and built_cv != live_cv:
        notes.append(f"built against ComfyUI {built_cv}, host is {live_cv}")
    built_pins = stamp.get("host_derived") or {}
    for pkg, was in sorted(built_pins.items()):
        now = (host_derived or {}).get(pkg)
        if was and now and was != now:
            notes.append(f"{pkg} pinned {was} at build, host now {now}")
    if not notes:
        return None
    return "; ".join(notes)
